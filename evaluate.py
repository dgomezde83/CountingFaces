...
def evaluate(weight_file_path,  output_dir=None, data_dir=None, img=None, list_imgs=None,
  ...
  with tf.Session() as sess:
    ...
    for filename in filenames:
      ...
      f_box = util.overlay_bounding_boxes(raw_img, refined_bboxes, lw, draw)
      #f_box = [[int(x) for x in r[:4] for r in refined_bboxes]]

      #modified code to see if it works
      f_box = refined_bboxes
      #we iterate through each box list
      print(range(len(refined_bboxes)))
      print(range(len(refined_bboxes[0])))
      for i in range(len(refined_bboxes)):
          for j in range(len(refined_bboxes[i])):
            f_box[i][j] = int(refined_bboxes[i][j])
      ...
      if save:
        ...
        cv2.imwrite(os.path.join(output_dir, str(len(refined_bboxes)) + "_" + fname), raw_img)
